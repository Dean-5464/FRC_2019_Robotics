...
public class Robot extends IterativeRobot {
	...
	// Joysticks
	Joystick Arcade = new Joystick(3);
	...
	// SpeedControllerGroup Intake_Motors = new SpeedControllerGroup(Intake_Left,
	// Intake_Right);
	...
	DoubleSolenoid Intake_Arm = new DoubleSolenoid(4, 5);
	DoubleSolenoid Intake = new DoubleSolenoid(3, 7);
	...
	private static final String kController_Drive = "Controller Drive";
	private static final String kJoystick_Drive = "Joystick Drive";

	private String m_autoSelected;
	private String d_DriveSelected;

	private SendableChooser<String> m_chooser = new SendableChooser<>();
	private SendableChooser<String> d_chooser = new SendableChooser<>();
	...
	public void robotInit() {
		...
		d_chooser.addDefault("Joystick Drive", kJoystick_Drive);
		...
		// New Chooser Non-default
		d_chooser.addObject("Controller Drive", kController_Drive);

		SmartDashboard.putData("Drive Choices", d_chooser);
	...
	public void autonomousPeriodic() {
		...
		case kAutoLeft:
			...
			// Scale Priority
			/*
			 * if (left_Color_Scale == true && step == 0) { step = 2; }else if
			 * (left_Color_Switch == true && left_Color_Scale == false && step == 0) { step
			 * = 7; }else if (left_Color_Switch == false && left_Color_Scale == false &&
			 * step == 0) { step = 11; }
			 */
			...
			while (Auto_Stop == false && DriverStation.getInstance().getMatchTime() > 0.5) {
				...
				// Drive Forward From The Starting Position To A Position Perpendicular To The
				// Scale
				case 2:
				...
				// Drive Forwards From The Starting Position To A Position Perpendicular To The
				// Switch
				case 7:
		...
		case kAutoRight:
			...
			// Scale Priority
			/*
			 * if (right_Color_Scale == true && step == 0) { step = 2; }else if
			 * (right_Color_Switch == true && right_Color_Scale == false && step == 0) {
			 * step = 7; }else if (right_Color_Switch == false && right_Color_Scale == false
			 * && step == 0) { step = 11; }
			 */
		...
		default:
			if(DriverStation.getInstance().getMatchTime() > 13) {
				Drive.tankDrive(DriveSpeed - Steer, DriveSpeed + Steer);
			}else if (DriverStation.getInstance().getMatchTime() <= 13){
				Drive.tankDrive(0.0, 0.0);
				Auto_Stop = true;
			}else {
				Drive.stopMotor();
	...
	public void teleopPeriodic() {
		...
		d_DriveSelected = d_chooser.getSelected();

		System.out.println("Drive Selected" + d_DriveSelected);

		while (1 == 1) {
//DriverStation.getInstance().getMatchTime() > 0.0
			switch (d_DriveSelected) {

			case kController_Drive:
				...
				Drive.tankDrive(Arcade.getRawAxis(3) * 0.8, Arcade.getRawAxis(1) * 0.8);

				// Shifting Command
				if (Arcade.getRawButton(7)) {
					Shifting.set(DoubleSolenoid.Value.kForward);
				} else if (Arcade.getRawButton(8)) {
				...
				// Intake Claw Command
				if (Arcade.getRawButton(5)) {
					Intake.set(DoubleSolenoid.Value.kForward);
				} else if (Arcade.getRawButton(6)) {
					Intake.set(DoubleSolenoid.Value.kReverse);
				} else {
				...
				// Lift Command
				if (Actuators.getRawButton(2)) {
					Lift_Motor.set(0.75);
				} else if (Actuators.getRawButton(3)) {
					Lift_Motor.set(-0.75);
				} else {
					Lift_Motor.set(0.0);
				}
				...
				/*
				 * if (Arcade.getRawButton(8) && Arcade.getRawButton(8)) { Climb_One.set(1.0);
				 * Climb_Two.set(1.0);
				 */
				if (Actuators.getRawButton(11)) {
					Climb_One.set(-1.0);
					Climb_Two.set(-1.0);
				}else if(Actuators.getRawButton(7)) {
					Climb_One.set(1.0);
					Climb_Two.set(1.0);
				} else {
					Climb_One.set(0.0);
			...
			default:
				...
				if (Actuators.getRawButton(1)) {
					Climb_One.set(-1.0);
					Climb_Two.set(-1.0);
				} else if (Actuators.getRawButton(11)) {
					Climb_One.set(1.0);
					Climb_Two.set(1.0);
				} else {
					Climb_One.set(0.0);
